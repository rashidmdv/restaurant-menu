...
type CatalogSubCategoryForm = z.infer<typeof formSchema>
...
export function subcategoriesMutateDrawer({ open, onOpenChange, currentRow }: Props) {
  ...
  const onSubmit = async (data: CatalogSubCategoryForm) => {
    try {
      setLoading(true)
    ...
    <Sheet open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) form.reset() }}>
      <SheetContent className="flex flex-col overflow-y-auto">
        <SheetHeader className="text-left">
          <SheetTitle>{isUpdate ? 'Update' : 'Create'} Catalog  Sub-Category</SheetTitle>
          <SheetDescription>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. Brakepads" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} placeholder="UUID of parent sub-category" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
