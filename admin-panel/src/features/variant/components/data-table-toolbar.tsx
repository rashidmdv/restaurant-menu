...
export function DataTableToolbar<TData>({
  ...
  const { models, filters, setFilters, refreshVariants } = useVariants()
  ...
  // Initialize model filters from API filters
  useEffect(() => {
    if (filters.modelId && !table.getColumn('modelId')?.getFilterValue()) {
      table.getColumn('modelId')?.setFilterValue([filters.modelId])
    }
  }, [filters.modelId, table])
  ...
  // Generate model options from available model
    ...
    <div className='flex items-center justify-between'>
      <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
        ...
        <div className='flex gap-x-2 flex-wrap'>
          ...
          {table.getColumn('modelId') && modelOptions.length > 0 && (
            <DataTableFacetedFilter
