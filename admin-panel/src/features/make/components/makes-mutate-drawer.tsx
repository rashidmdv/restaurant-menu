...
export function MakesMutateDrawer({ open, onOpenChange, currentRow }: Props) {
  ...
  const onSubmit = async (data: VehicleMakeForm) => {
    try {
      setLoading(true)
      ...
      if (isUpdate) {
        ...
        //   `https://supplier-catalog-service-dev.run.app/vehicle-makes/${currentRow.id}`,
        //   formattedData
        // )
        showSubmittedData(
        ...
        //   'https://supplier-catalog-service-dev.run.app/vehicle-makes',
        //   formattedData
        // )
        showSubmittedData(
          { ...formattedData, id: `MDL-${Math.floor(Math.random() * 10000)}` },
          'New vehicle make created:'
        )
      }
    ...
    <Sheet open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) form.reset() }}>
      <SheetContent className="flex flex-col overflow-y-auto">
        <SheetHeader className="text-left">
          <SheetTitle>{isUpdate ? 'Update' : 'Create'} Vehicle Make</SheetTitle>
          <SheetDescription>
